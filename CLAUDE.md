...
| **Pin GitHub Actions to SHAs** | `actions/checkout@v4` and friends are mutable tags — a compromised tag reaches CI, which holds the deploy secrets. Python deps are pinned to exact versions; a hash-pinned lockfile (`pip-compile --generate-hashes`) is the step after that. |
