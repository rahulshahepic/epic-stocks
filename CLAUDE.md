...
- **Never key a rate limit on `request.client.host`.** Behind Caddy that is the proxy's address for every caller, collapsing the limit into one shared bucket. Use `client_ip()` from `backend/scaffold/client_ip.py`, which unwraps `X-Forwarded-For` only as far as `TRUSTED_PROXY_HOPS` allows.
