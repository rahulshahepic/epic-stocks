...
- **backend/app/epic_import/ imports Epic's own files** (share summary CSV, Stock Loan Statement PDF). Every derived field is tagged with the id of the rule in `rules.py` that produced it; `reconcile.py` diffs a derivation against exported data so wrong rules can be identified by id. Keep the rule ids stable — they are how import bugs get reported.
