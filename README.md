...
- **Epic exit value + additional portfolio** ($M) — by default the simulator treats your non-Epic wealth as a single "Additional portfolio" number with full tax basis, like the Epic exit (which was just sold). Click **Account types & basis (advanced)** to break it down into:
  - **Taxable cost basis** — the basis of pre-existing taxable brokerage. Withdrawals from the taxable bucket realize LTCG only on the gain portion (basis is returned tax-free); Epic exit proceeds enter this bucket with full basis, no embedded gain.
  - **Traditional 401(k) / IRA** — locked until age 59½. Withdrawals are taxed as **ordinary income**. Pre-59½ ruin is possible even with this bucket full — the bridge years must come from cash + taxable.
  - **Roth IRA / 401(k)** — locked until age 59½. Withdrawals are **tax-free**.
- **Stocks / Bonds / Cash allocation** (%) — split the total portfolio. Cash is auto-computed as `100 − stocks − bonds`. The cash buffer is funded from the taxable bucket only (we don't model cash held inside tax-deferred accounts as separately accessible); if taxable can't fully fund the cash allocation, the rest stays invested in the tax-deferred buckets and a small warning surfaces in the advanced view.
