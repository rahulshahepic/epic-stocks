...
- **Taxes** — computed each year from 2026 federal brackets (ordinary + LTCG, MFJ when spouse is included), **NIIT** (3.8% on investment income above MAGI thresholds), Medicare **IRMAA** after 65, and the state rates from **Settings → Tax Rates**. No user input — the simulator pulls everything it needs from your tax settings + filing status + the year's actual withdrawals. Withdrawal order each year: cash → taxable (LTCG on gain only) → traditional → Roth, with traditional and Roth gated until 59½. Cost basis is tracked through the path: each year it's eroded by that sampled year's actual CPI from the historical bootstrap (basis is fixed in nominal $ in real life but the sim runs in real $, so basis loses purchasing power over time — high-inflation years boost real returns *and* erode basis simultaneously). When the cash buffer gets refilled from taxable equity gains, the gain portion realizes LTCG on top of the year's spending tax — refills are not free.
  - **Wisconsin assumption.** This app targets Epic employees (Verona, WI), so the simulator applies WI's Social Security exemption: 85% of SS counts toward federal taxable income (per IRS rules) but state tax is computed on traditional withdrawals only, not on SS. If your sim is being used outside WI, this slightly under-taxes filers in the 12 states that *do* tax SS (CO, CT, KS, MN, MO, MT, NE, NM, RI, UT, VT, WV) and is correct everywhere else. Wisconsin's 30% LTCG exclusion isn't applied automatically — set your `state_lt_cg_rate` in Tax Rates to your effective post-exclusion rate (e.g., 7.65% × 0.70 = ~5.36% at the WI top bracket).
