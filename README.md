...
- **Taxes** — computed each year from 2026 federal brackets (ordinary + LTCG, MFJ when spouse is included), **NIIT** (3.8% on investment income above MAGI thresholds), Medicare **IRMAA** after 65, and Wisconsin progressive brackets on traditional 401(k)/IRA withdrawals (see WI assumption below). Withdrawal order each year: cash → taxable (LTCG on gain only) → traditional → Roth, with traditional and Roth gated until 59½. Cost basis is tracked through the path: each year it's eroded by that sampled year's actual CPI from the historical bootstrap (basis is fixed in nominal $ in real life but the sim runs in real $, so basis loses purchasing power over time — high-inflation years boost real returns *and* erode basis simultaneously). When the cash buffer gets refilled from taxable equity gains, the gain portion realizes LTCG on top of the year's spending tax — refills are not free.
