...
**Shares nothing accounts for become a sale you finish filling in.** Whatever the down-payment arithmetic cannot explain was sold, so the import drafts a sale for it. The workbook gives the share count and nothing else — it carries no sale date and no price — so the wizard adds a **Shares that were sold** step that asks you for those two, with the count already filled in. Leave a row blank and it simply is not imported; nothing is ever guessed, because a made-up sale price becomes a capital gain you cannot tell apart from a real one. If you take the assistant route instead, the prompt tells it to ask you for the same two figures rather than infer them.
...
- **A figure only the user has is asked for, never inferred.** What the down-payment arithmetic cannot explain was sold, and the workbook gives the share count but no date and no price. So the import drafts the sale with the count filled in and those two blank, and both repair routes converge on asking: the wizard adds a screen, and the assistant prompt is told to ask rather than infer. An invented sale price is worse than a missing one — it reaches capital gains looking exactly like a real figure, and rule 7's "do not estimate anything" would otherwise be quietly relaxed the one place it matters most. `C12` keeps an unanswered sale visible, and the wizard is what refuses to write one.
