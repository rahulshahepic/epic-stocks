...
- **A parser reached without a session gets bounded twice.** `POST /api/trial/analyze` takes no account at all, so the PDF and CSV parsers behind it are the cheapest expensive thing an anonymous caller can reach. Text extraction is per page and a file declares its own page count, so `MAX_PDF_PAGES` caps it; a malformed figure in a statement row, a CSV field past the `csv` module's 128 KB limit, or a grant year outside 1900–2100 all become findings rather than exceptions. That last part matters beyond tidiness: an unhandled exception is a 500, every 500 writes an `error_logs` row, and the nightly job trims that table to 500 rows — so cheap 500s flush real tracebacks out of the window an admin debugs from.
