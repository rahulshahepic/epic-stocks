...
| `VITE_API_BASE` | No | Absolute origin the frontend calls for `/api` requests, baked in at Vite build time. Leave unset for the PWA — the SPA and the API are served from one domain, so requests stay relative. Only a native shell build (whose WebView origin is not the server) needs it, e.g. `https://equity.example.com`. |
...
- **The frontend never assumes it is same-origin with the API.** Every request resolves through `apiUrl()` in `frontend/src/config.ts`, and `apiFetchRaw` in `api.ts` is the only place `fetch` is called. `API_BASE` is empty by default, so the PWA keeps issuing relative `/api` requests exactly as before; a build that sets `VITE_API_BASE` points the same code at an absolute origin.
