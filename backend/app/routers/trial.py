...
def analyze(
    ...
    check_rate_ip_shared(client_ip, "trial_analyze", max_calls=10, window_secs=300)
...
def save_intent(request: Request, db: Session = Depends(get_db)):
    ...
    from scaffold.client_ip import client_ip as _client_ip
    client_ip = _client_ip(request)
    check_rate_ip_shared(client_ip, "trial_save_intent", max_calls=20, window_secs=300)
