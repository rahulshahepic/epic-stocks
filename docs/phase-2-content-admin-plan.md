# Phase 2 plan: Content admin role + editable grant-program content
...
Phase 1 moved every wizard constant (grant templates, loan rates, loan refinances, bonus schedule variants, grant-type metadata, DP-shares year cutoff, fallback tax rates, etc.) into six DB tables — `grant_templates`, `grant_type_defs`, `bonus_schedule_variants`, `loan_rates`, `loan_refinances`, `grant_program_settings` — exposed via read-only `GET /api/content` and consumed by `ImportWizard.tsx`. Seed data was preloaded with the exact Epic values — wizard behavior is byte-for-byte unchanged. **No new role, no edit UI, no write endpoints.**
...
- Content admins can CRUD all tables created in Phase 1: grant templates, grant type defs, bonus variants, loan rates, loan refinances, grant-program settings.
...
2. `grant_program_settings.flexible_payoff_enabled` — `Boolean, default=False, server_default='0'`. Migrate existing value from `system_settings` key `'flexible_payoff_enabled'` if present (may not exist — default to False). Leave the old row intact for one release, then drop it in a follow-up.
...
- `backend/app/routers/content.py` — extend with:
  ...
  - Same CRUD shape for: `/grant-type-defs/{name}`, `/bonus-schedule-variants/{id}`, `/loan-rates/{id}`, `/loan-refinances/{id}`.
  - `PUT /api/content/grant-program-settings` — partial update for the singleton row; also accepts `flexible_payoff_enabled`.
...
- `backend/scaffold/routers/admin.py:463-476` — current `GET/POST /api/admin/flexible-payoff` keeps its shape but reads/writes `GrantProgramSettings.flexible_payoff_enabled` via the content service instead of `system_settings`. Preserves existing admin UI behavior.
...
- Same toggle is also exposed in `PUT /api/content/grant-program-settings` so content admins (not only admins) can change it from `/content`.
...
- `frontend/src/app/pages/Content.tsx` (new). Tabbed layout:
  ...
  5. **Loan Refinances** — grouped by `(chain_kind, grant_year, grant_type)`; within each group, rows are reorderable (order_idx); `orig_due_date` only for `tax`.
  6. **Program Settings** — single form for the `grant_program_settings` singleton. Includes the `flexible_payoff_enabled` toggle with explanatory helper text ("When off, loan-payoff sales are forced to the same-tranche method regardless of user preference").
...
- `frontend/src/api.ts` — extend the `api` object with:
  ...
  - `updateGrantTemplate`, `createGrantTemplate`, `deleteGrantTemplate`, plus analogous methods for the other five tables, plus `updateGrantProgramSettings`.
...
- `backend/tests/test_content.py` — extend with:
  ...
  - `flexible_payoff_enabled` toggle via `PUT /api/content/grant-program-settings` and via `POST /api/admin/flexible-payoff` should both modify the same underlying field.
- `backend/tests/test_admin.py` — extend with promote/demote tests, and `GET /api/admin/users` returns `is_content_admin`.
- `backend/tests/test_flexible_payoff.py` — update to confirm the toggle now reads from `grant_program_settings`.
...
- README.md updates per the mandatory checklist in `CLAUDE.md`:
  ...
  - Note the behavior change: `flexible_payoff_enabled` now lives in `grant_program_settings`.
