...
function mockFetch(responses: Record<string, unknown>) {
  vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
    ...
    if (url.includes('/api/admin/mcp/usage')) return new Response(JSON.stringify({ users: [], tools: [], calls_24h: 0, calls_7d: 0, calls_30d: 0, errors_7d: 0, denied_7d: 0, audit_rows: 0 }), { status: 200, headers: { 'Content-Type': 'application/json' } })
