...
function Field({
 ...
 className={`mt-0.5 block w-full rounded-md border border-cs-border-strong bg-cs-surface px-2 py-1.5 text-xs text-cs-text${disabled ? ' opacity-50 cursor-not-allowed' : ''}`}
 />
 </label>
 )
}
...
function PercentField({ label, value, onChange, hint, placeholder, className }: {
 ...
 className={className ?? "mt-0.5 block w-full rounded-md border border-cs-border-strong bg-cs-surface px-2 py-1.5 text-xs text-cs-text"}
 />
 )
...
function LoanReviewRow({ loan, onChange }: { loan: ReviewedLoan; onChange: (l: ReviewedLoan) => void }) {
 ...
 <span className="text-cs-muted">{ratePercent}%</span>
 </div>
 {loan.enabled && (
 ...
 * etc.) becomes a blocking issue — surfaced per row, submit disabled. */
 ...
 const isPreTaxGrant = !isNaN(enteredPrice) && enteredPrice === 0
 ...
 <p className="mt-1 text-[11px] text-cs-muted">{GRANT_DESCRIPTIONS[grantDraft.type]}</p>
 </div>
 ...
 return (
 <div key={row.year} className="rounded-md border border-cs-border">
 ...
 return (
 <div key={`${row.year}-${row.type}`} className="rounded-md border border-cs-border p-3 space-y-3">
 <div className="flex items-center gap-2">
 <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${GRANT_COLORS[row.type]}`}>{row.year} {row.type}</span>
 {row.type !== 'Free' && <span className="text-[11px] text-cs-muted">{row.periods} periods from {fmtDate(row.vest_start)}</span>}
 {row.type === 'Free' && <span className="text-[11px] text-cs-muted">vests {fmtDate(row.vest_start)}</span>}
 </div>
 {row.isBonus2020 && (
 <div>
 ...
 <span className="ml-1 text-[11px] text-cs-muted self-center">
 ...
 <BackBtn onClick={back} />
 <div>
 <h2 className="text-base font-semibold text-cs-text">Tax loans</h2>
 <p className="mt-1 text-xs text-cs-muted">
 ...
 <BackBtn onClick={back} />
 <div>
 <h2 className="text-base font-semibold text-cs-text">Interest loans</h2>
 <p className="mt-1 text-xs text-cs-muted">
