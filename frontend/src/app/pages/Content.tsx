...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
 ...
 <button onClick={onClose} aria-label="Close dialog" className="text-cs-text-2 hover:text-cs-text-2 ">✕</button>
 </div>
 {children}
 </div>
 </div>
...
function FormActions({ mode, busy, onCancel, onDelete }: { mode: Mode; busy: boolean; onCancel: () => void; onDelete?: () => void }) {
 ...
 <button type="submit" disabled={busy} className="rounded-md bg-cs-brand px-3 py-1.5 text-xs font-medium text-white hover:bg-cs-brand-hover disabled:opacity-50">Save</button>
 <button type="button" onClick={onCancel} disabled={busy} className="rounded-md border border-cs-border-strong px-3 py-1.5 text-xs font-medium text-cs-text-2 hover:bg-cs-raised disabled:opacity-50 ">Cancel</button>
 {mode === 'edit' && onDelete && (
 <button type="button" onClick={onDelete} disabled={busy} className="ml-auto rounded-md border border-rose-300 px-3 py-1.5 text-xs font-medium text-cs-brand hover:bg-rose-50 disabled:opacity-50 dark:border-rose-900 dark:hover:bg-rose-950">Delete</button>
 )}
 </div>
 )
}
...
function AddButton({ onClick, label }: { onClick: () => void; label: string }) {
 return (
 <button onClick={onClick} className="rounded-md bg-cs-brand px-3 py-1.5 text-xs font-medium text-white hover:bg-rose-800">
 + {label}
 </button>
 )
}
...
function RowCount({ n, noun }: { n: number; noun: string }) {
 return <p className="text-xs text-cs-text-2">{n} {noun}{n === 1 ? '' : 's'}</p>
}
...
export default function Content() {
 ...
 if (!blob) {
 return <p className="text-xs text-cs-text-2">Loading content…</p>
 }
 ...
 return (
 <div className="space-y-4 text-cs-text">
 <div>
 <h2 className="text-sm font-semibold text-cs-text">Grant-program content</h2>
 ...
 <nav className="flex flex-wrap gap-1 border-b border-cs-border pb-2 ">
...
function TemplatesTab({ blob, wrap, busy }: { blob: ContentBlob; wrap: WrapFn; busy: boolean }) {
 ...
 <div className="overflow-x-auto rounded-md border border-cs-border">
 ...
 key={t.id}
 onClick={() => openEdit(t)}
 className="cursor-pointer border-t border-cs-border hover:bg-cs-raised "
 >
...
function RatesTab({ blob, wrap, busy }: { blob: ContentBlob; wrap: WrapFn; busy: boolean }) {
 ...
 value={draftKind}
 disabled
 className="w-full rounded-md border border-cs-border-strong bg-cs-raised px-2 py-1.5 text-xs text-cs-muted "
 />
 </Field>
 {draftKind === 'tax' && (
...
function SettingsTab({ blob, wrap, busy }: { blob: ContentBlob; wrap: WrapFn; busy: boolean }) {
 ...
 <span className="mb-1 font-medium text-cs-text-2">Federal tax fallback (%)</span>
 ...
 <label className="flex flex-col text-xs">
 <span className="mb-1 font-medium text-cs-text-2">State tax fallback (%)</span>
 ...
 <label className="flex flex-col text-xs">
 <span className="mb-1 font-medium text-cs-text-2">Min DP (% of purchase)</span>
 ...
 <label className="flex flex-col text-xs">
 <span className="mb-1 font-medium text-cs-text-2">Min DP cap ($)</span>
 <TextInput type="number" step="1000" value={form.dp_min_cap} onChange={e => update({ dp_min_cap: Number(e.target.value) })} />
 </label>

 <div className="col-span-full rounded-md border border-cs-border p-3 text-[11px] text-cs-text-2 ">
 <span className="font-medium text-cs-text-2 ">Derived price years: </span>
 ...
 <div className="col-span-full rounded-md border border-cs-border p-3 text-xs ">
 ...
 />
 <span>
 <span className="font-medium text-cs-text-2 ">Flexible loan-payoff methods</span>
