...
function VestingTaxCard({ e, ts }: { e: TimelineEvent; ts: TaxSettings }) {
 ...
 <h3 className="mb-3 text-sm font-semibold text-cs-text">Estimated Tax</h3>
...
function Unrealized83bCard({ e, ts }: { e: TimelineEvent; ts: TaxSettings }) {
 ...
 <h3 className="mb-3 text-sm font-semibold text-cs-text">83(b) Election — Gain if you sold today</h3>
 <div className="space-y-1">
 ...
 />
 {isFuture && (
 <p className="pt-1 text-cs-text-2">You only owe this tax when you sell. The long-term clock starts at vesting.</p>
 )}
 </div>
 </div>
 )
...
export default function Events() {
 ...
 {EVENT_TYPES.map(t => (
 <label key={t} className="flex cursor-pointer items-center gap-2 px-3 py-1.5 hover:bg-cs-raised">
 <input
 type="checkbox"
 ...
 />
 <span className="flex-1 text-xs text-cs-text-2">{t}</span>
 <span className="text-[10px] text-cs-text-2">{events.filter(e => e.event_type === t).length}</span>
 </label>
 ))}
 </div>
 )}
 ...
 <span className="text-cs-text-2">{e.date}</span>
 ...
 {sharesDisplay && (
 <span className={`tabular-nums font-medium ${e.event_type === 'Sale' ? 'text-red-600 dark:text-red-400' : 'text-cs-text-2'}`}>
 ...
 {isMobileExpanded && (
 <div className="mt-2 space-y-1 border-t border-cs-border pt-2 ">
 <div className="flex justify-between">
 <span className="text-cs-muted">Price</span>
 <span className="tabular-nums text-cs-text-2">{fmtPrice(e.share_price)}</span>
 </div>
 {(is83b || e.income > 0) && (
 <div className="flex justify-between">
 <span className="text-cs-muted">Income</span>
 {is83b
 ...
 {capGainsLabel && (
 <div className="flex justify-between">
 <span className="text-cs-muted">{capGainsLabel}</span>
 ...
 <div className="flex justify-between">
 <span className="text-cs-muted">Shares held</span>
 <span className="tabular-nums font-medium text-cs-text">{fmtNum(e.cum_shares)}</span>
 </div>
 ...
 >
 <td className="whitespace-nowrap px-3 py-2 text-cs-text-2">{e.date}</td>
 ...
 <td className={`px-3 py-2 text-right ${e.event_type === 'Sale' ? 'text-red-600 opacity-70 dark:text-red-400' : 'text-cs-text-2'}`}>
 {e.event_type === 'Early Loan Payment'
 ? (e.amount != null ? fmt$(e.amount) : '—')
 : fmtNum(e.vested_shares ?? e.granted_shares)}
 </td>
 <td className="px-3 py-2 text-right text-cs-text-2">{fmtPrice(e.share_price)}</td>
 ...
 <td className="px-3 py-2 text-right font-medium text-cs-text">{fmtNum(e.cum_shares)}</td>
 </tr>
 {isSaleExpanded && bd && (
 <tr className="bg-cs-surface">
