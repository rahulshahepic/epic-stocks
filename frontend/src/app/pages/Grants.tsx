        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          {form.type === 'Purchase' && (
            <label className="block">
              ...
              <p className="text-[10px] text-gray-400 dark:text-slate-500">Bonus shares used first, then oldest (non-taxable exchange)</p>
              <input
                type="number"
                      ...
                      <td colSpan={9} className="px-3 pb-3 pt-0">
                        <div className="rounded-md bg-stone-50 p-3 dark:bg-slate-800">
                          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                            <div><span className="text-gray-400 dark:text-slate-500">Amount</span> <span className="ml-1 font-medium text-gray-700 dark:text-slate-200">{fmtPrice(loan.amount)}</span></div>
                            <div><span className="text-gray-400 dark:text-slate-500">Rate</span> <span className="ml-1 font-medium text-gray-700 dark:text-slate-200">{(loan.interest_rate * 100).toFixed(2)}%</span></div>
                            <div><span className="text-gray-400 dark:text-slate-500">Due</span> <span className="ml-1 font-medium text-gray-700 dark:text-slate-200">{loan.due_date}</span></div>
                            <div><span className="text-gray-400 dark:text-slate-500">Loan #</span> <span className="ml-1 font-medium text-gray-700 dark:text-slate-200">{loan.loan_number ?? '—'}</span></div>
                          </div>
                          {linkedSale ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-sm rounded-lg bg-white p-5 shadow-xl dark:bg-slate-900">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={closeSellModal} aria-label="Close dialog" className="text-gray-400 hover:text-gray-600 dark:hover:text-slate-300">✕</button>
            </div>
