...
export default function Grants() {
  ...
  if (mode !== 'list') {
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          {form.type === 'Purchase' && (
            <label className="block">
              <span className="text-xs text-cs-muted">Shares used as down payment</span>
        ...
        {showLoanSection && (
          <>
            <h3 className="pt-1 text-sm font-medium text-cs-text-2">
          ...
          <div className="flex gap-2">
            ...
            {mode !== 'edit' && (
              <button
                ...
                disabled={saving}
                className="rounded-md bg-rose-50 px-3 py-1.5 text-xs font-medium text-cs-brand hover:bg-indigo-200 dark:bg-rose-900/40 dark:text-rose-300 dark:hover:bg-rose-900/50 disabled:opacity-50"
              >
                Save & Add Another
              </button>
            )}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-cs-text">Grants</h2>
        {!epicMode && (
          <div className="flex gap-2">
            <button onClick={openPurchase} className="rounded-full bg-cs-brand px-3 py-1 text-xs font-semibold text-white hover:bg-cs-brand-hover">
              + Purchase
            </button>
      ...
      {isMobile ? <div className="space-y-2">
        {grants.map(g => {
          ...
          return (
            <div key={g.id} className="rounded-xl border border-cs-border bg-cs-surface p-3 text-xs shadow-card">
              {/* Line 1: Year + Type + Action */}
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="font-medium text-cs-text-2">{g.year}</span>
                ...
                {readOnly ? null : epicMode ? (
                  ...
                  <button onClick={() => openEdit(g)} className="text-cs-brand hover:text-cs-brand-hover dark:hover:text-rose-300">Edit</button>
                )}
              </div>
              {/* Line 2: Shares + Cost Basis */}
              <div className="mt-1 text-cs-text-2">
                <span className="tabular-nums">{fmtNum(g.shares)}</span> shares <span className="text-cs-muted">&middot;</span> <span className="tabular-nums">{fmtPrice(g.price)}</span> basis
              </div>
              {/* Line 3: Vest details */}
              <div className="mt-1 text-cs-muted">
                Vest: {g.vest_start} <span className="text-cs-muted">&middot;</span> {g.periods} periods
                {g.exercise_date && <> <span className="text-cs-muted">&middot;</span> Ex: {g.exercise_date}</>}
              </div>
              {/* Line 4: Loan status */}
              <div className="mt-1">
                {loan ? (
                  ...
                  <span className="text-[10px] text-cs-text-2">&mdash;</span>
                )}
              </div>
              ...
              {isExpanded && loan && (
                <div className="mt-2 rounded-md bg-cs-raised p-3 ">
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                    <div><span className="text-cs-text-2">Amount</span> <span className="ml-1 font-medium text-cs-text-2">{fmtPrice(loan.amount)}</span></div>
                    <div><span className="text-cs-text-2">Rate</span> <span className="ml-1 font-medium text-cs-text-2">{(loan.interest_rate * 100).toFixed(2)}%</span></div>
                    <div><span className="text-cs-text-2">Due</span> <span className="ml-1 font-medium text-cs-text-2">{loan.due_date}</span></div>
                    <div><span className="text-cs-text-2">Loan #</span> <span className="ml-1 font-medium text-cs-text-2">{loan.loan_number ?? '\u2014'}</span></div>
                  </div>
                  {linkedSale ? (
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {grants.map(g => {
                ...
                return (
                  <>
                    <tr key={g.id} className="bg-cs-surface">
                      <td className="px-3 py-2 text-cs-text-2">{g.year}</td>
                      ...
                      <td className="px-3 py-2 text-right text-cs-text-2">{fmtNum(g.shares)}</td>
                      <td className="px-3 py-2 text-right text-cs-text-2">{fmtPrice(g.price)}</td>
                      <td className="px-3 py-2 text-cs-muted">{g.vest_start}</td>
                      <td className="px-3 py-2 text-right text-cs-muted">{g.periods}</td>
                      <td className="px-3 py-2 text-cs-muted">{g.exercise_date}</td>
                      <td className="px-3 py-2">
                        {loan ? (
                          <button
                            onClick={() => setExpandedGrantId(isExpanded ? null : g.id)}
                    ...
                    {isExpanded && loan && (
                      <tr key={`${g.id}-loan`} className="bg-cs-surface">
      ...
      <p className="text-xs text-cs-text-2">{grants.length} grants</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-sm rounded-lg bg-cs-surface p-5 shadow-xl ">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={closeSellModal} aria-label="Close dialog" className="text-cs-text-2 hover:text-cs-text-2 ">✕</button>
            </div>
            ...
            <div className="space-y-3">
              ...
              {sellEstimate && !sellEstimateLoading && (
                <div className="rounded-md bg-cs-raised p-3 text-xs ">
                  <div className="grid grid-cols-2 gap-y-1.5">
                    <span className="text-cs-muted">Shares needed</span>
                    <span className="text-right font-medium text-cs-text">{fmtNum(sellEstimate.shares_needed)}</span>
                    <span className="text-cs-muted">Gross proceeds</span>
                    <span className="text-right font-medium text-cs-text">{fmtPrice(sellEstimate.gross_proceeds)}</span>
                    <span className="text-cs-muted">Est. tax</span>
                    <span className="text-right font-medium text-red-600 dark:text-red-400">{fmtPrice(sellEstimate.estimated_tax)}</span>
                    <span className="text-cs-muted">Net proceeds</span>
                    ...
                    {sellEstimate.loan_balance != null && (
                      <>
                        <span className="text-cs-muted">Loan balance</span>
                        <span className="text-right font-medium text-cs-text">{fmtPrice(sellEstimate.loan_balance)}</span>
                        <span className="text-cs-muted">Covers loan</span>
