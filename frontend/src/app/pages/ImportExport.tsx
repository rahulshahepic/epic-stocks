...
export default function ImportExport() {
 ...
 <span className="text-sm font-semibold text-cs-text">Import from Excel</span>
 <span className="mt-1 text-xs text-cs-muted">
 ...
 <span className="ml-1 text-cs-muted ">(recommended)</span>
 <br />
 <span className="text-cs-muted ">For each loan, the app will plan a share sale big enough to cover the loan plus the tax on it.</span>
 </span>
 </label>
 </div>
 ...
 {isImporting && (
 <p className="mt-3 text-xs text-cs-muted">Uploading and processing...</p>
 )}
 ...
 className="rounded-md bg-rose-50 px-3 py-1.5 text-xs font-medium text-cs-brand hover:bg-rose-100 dark:bg-rose-900/40 dark:text-rose-300 dark:hover:bg-rose-900/50"
 >
 ...
 <div key={sheet}>
 <p className="text-xs font-semibold text-cs-text-2">{sheet} sheet</p>
 <dl className="mt-1 divide-y divide-gray-100 dark:divide-gray-800">
 {cols.map(({ col, desc }) => (
 <div key={col} className="flex gap-2 py-1.5">
 <dt className="w-32 shrink-0 font-mono text-xs text-cs-brand">{col}</dt>
 <dd className="text-xs text-cs-muted">{desc}</dd>
 </div>
 ))}
 </dl>
 </div>
